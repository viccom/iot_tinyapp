	...
	for {
		...
		case msg := <-ch:
			if msg.Error != nil {
				...
				fmt.Printf("[channel ] sub=%d ts=%s node=%s value=%v\n", sub.SubscriptionID(), msg.ServerTimestamp.UTC().Format(time.RFC3339), msg.NodeID, msg.Value.Value())
			}
			time.Sleep(lag)
		}
	}
...
func main() {
	var (
		...
		tagsFile = flag.String("tagsfile", "", "Path to nodeIDS tags.csv.")
		nodeID   = flag.String("node", "ns=1;s=Tag1,ns=1;s=Tag2,ns=1;s=CurrentTime", "node id to subscribe to")
	...
	if len(*tagsFile) == 0 {
		tags = splitStringByComma(*nodeID)
	} else {
		fmt.Println("tagsFile is not empty")
	}
	...
	wg.Add(1)
	go startCallbackSub(ctx, m, *interval, 0, wg, tags...)

	// start channel-based subscription
	wg.Add(1)
	go startChanSub(ctx, m, *interval, 0, wg, tags...)
